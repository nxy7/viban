defmodule Viban.Kanban.Actors.HookSystemComprehensiveTest do
  ...
  describe "Task Movement During Hook Execution" do
    ...
    test "programmatic move interrupts hooks same as manual move", %{
      ...
      assert {:ok, _} =
               wait_for_hook_status(task.id, "Hook Before Programmatic Move", :cancelled),
             "Hook should have been cancelled"

      {:ok, executions} = HookExecution.history_for_task(task.id)
      hook_execs = Enum.filter(executions, &(&1.hook_name == "Hook Before Programmatic Move"))

      cancelled_exec = Enum.find(hook_execs, &(&1.status == :cancelled))
      assert cancelled_exec, "Expected at least one cancelled execution for the hook"
      assert cancelled_exec.skip_reason in [:column_change, :user_cancelled]
  ...
  describe "Transparent hook behavior" do
    ...
    test "mix of transparent and non-transparent hooks respects error state", %{
      ...
      assert {:ok, transparent_exec} =
               wait_for_hook_status(task.id, "Transparent Hook", :completed)
