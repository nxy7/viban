defmodule VibanWeb.HomeLive do
      ...
      <div class="flex min-h-full items-center justify-center p-4">
        ...
        <div class="relative transform overflow-hidden rounded-xl bg-gray-900 border border-gray-800 shadow-xl transition-all w-full max-w-md p-6">
          ...
          <form phx-submit="create_board" class="space-y-4">
            <div>
              <label for="board_name" class="block text-sm font-medium text-gray-300 mb-1">
                Name *
              </label>
              <input
                type="text"
            ...
            <div>
              <label class="block text-sm font-medium text-gray-300 mb-1">GitHub Repository *</label>
              ...
              <% else %>
                <div class="space-y-2">
                  ...
                  <div class="max-h-48 overflow-y-auto bg-gray-800 border border-gray-700 rounded-lg">
                    <%= if @repos_loading do %>
                    ...
                    <%= if !@repos_loading && !@repos_error && @repos == [] do %>
                      <div class="p-4 text-center text-gray-400">
                        No repositories found
                      </div>
                    <% end %>

                    <%= for repo <- filter_repos(@repos, @repo_search) do %>
                      <button
                        type="button"
            ...
            <div class="flex gap-3 pt-2">
              ...
              <button
                type="submit"
                disabled={@selected_repo == nil}
                class="flex-1 px-4 py-2 bg-brand-600 hover:bg-brand-700 disabled:bg-brand-800 disabled:cursor-not-allowed text-white font-medium rounded-lg transition-colors"
              >
                Create Board
              </button>
            </div>
  ...
  defp humanize_repo_name(name) do
    name
    |> String.replace(~r/[-_]/, " ")
    |> String.split()
  ...
  def handle_event("select_repo", %{"repo_id" => repo_id}, socket) do
    ...
    board_name = humanize_repo_name(repo.name)

    socket =
      socket
      |> assign(:selected_repo, repo)
      |> push_event("set_board_name", %{name: board_name})
  ...
  def handle_info(:load_repos, socket) do
    ...
    if user && user.access_token do
      case VCS.list_repos(:github, user.access_token) do
        {:ok, repos} ->
          socket =
            socket
            |> assign(:repos, repos)
        ...
        {:error, reason} ->
          Logger.error("Failed to load repos: #{inspect(reason)}")

          socket =
            socket
            |> assign(:repos_error, "Failed to load repositories")
            |> assign(:repos_loading, false)
      ...
      socket =
        socket
        |> assign(:repos_error, "Not authenticated")
        |> assign(:repos_loading, false)
