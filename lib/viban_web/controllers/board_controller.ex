defmodule VibanWeb.BoardController do
  ...
  Handles board creation with VCS repository selection.
  ...
  - `repo` (required) - Object with VCS repository info:
    ...
    - `local_path` - Path to local repository (optional, can be set later)
  ...
  defp build_repo_params(repo, provider) do
    ...
    %{
      ...
      local_path: get_param(repo, :local_path)
    }
  end
  ...
  defp create_repository(repo_params, board_id, provider) do
    case Repository.create(%{
           ...
           local_path: repo_params.local_path
         }) do
      {:ok, repository} ->
        {:ok, repository}
