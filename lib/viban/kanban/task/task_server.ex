defmodule Viban.Kanban.Task.TaskServer do
  ...
  @impl true
  def handle_cast({:column_changed, new_column_id, _new_position}, state) do
    column_changed = state.current_column_id != new_column_id

    if column_changed do
      Logger.info("Column changed to #{new_column_id}", task_id: state.task_id)
      ...
      {:noreply, %{state | current_column_id: new_column_id}}
    else
      Logger.debug("Task already in column #{new_column_id}, skipping hook re-queue",
