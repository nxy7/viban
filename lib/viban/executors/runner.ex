defmodule Viban.Executors.Runner do
  ...
  def init(opts) do
    ...
    continue_session = Keyword.get(opts, :continue_session, false)
  ...
  def handle_info(:start_executor, state) do
    ...
    executor_opts =
      ...
      |> Keyword.put(:continue_session, continue_session)

    {executable, args} = executor_module.build_command(enhanced_prompt, executor_opts)

    IO.puts("[Runner] Starting #{executor_module.name()} for task #{task_id}: #{executable} #{Enum.join(args, " ")}")

    IO.puts("[Runner] Working directory: #{inspect(working_directory)}")

    port_opts = build_port_options(args, working_directory, executor_module)

    case find_executable(executable) do
      {:ok, path} ->
        IO.puts("[Runner] Executable path: #{path}")
        IO.puts("[Runner] Port options: #{inspect(port_opts)}")
  ...
  def handle_info({port, {:exit_status, exit_code}}, %{port: port} = state) do
    ...
    if exit_code != 0 do
      save_message(task_id, session_id, :system, "Failed with exit code #{exit_code}")
    end
  ...
  defp init_with_executor(executor_module, task_id, executor_type, prompt, working_directory, images, continue_session) do
  ...
  defp build_environment(executor_module) do
    ...
    base_env = [
      ...
      {~c"XDG_CONFIG_HOME", String.to_charlist(System.get_env("XDG_CONFIG_HOME") || Path.join(home, ".config"))}
    ]
