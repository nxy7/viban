...
import type { KanbanBoard, KanbanColumn, KanbanTask, KanbanHook, KanbanColumnHook, KanbanRepository, KanbanTaskEvent, KanbanPeriodicalTask, KanbanTaskTemplate, StateServerActorState } from './schema';
...
export const syncTaskTemplatesCollection = createCollection(
  electricCollectionOptions<KanbanTaskTemplate>({
    id: 'sync_task_templates',
    getKey: (item) => item.id,
    shapeOptions: {
      url: getUrl('/api/sync/'),
      params: {query: 'sync_task_templates'},
    },
  }),
);
