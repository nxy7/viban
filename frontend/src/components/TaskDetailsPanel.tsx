...
import { getStoredBoolean, setStoredBoolean } from "~/lib/storageUtils";

const log = createLogger("TaskDetails");
...
type TaskCreatedActivityItem = {
  type: "task_created";
  timestamp: string;
  description: string | null;
};

type OutputActivityItem = {
  type: "output";
  line: OutputLine;
};

type GroupedHooksActivityItem = {
  type: "grouped_hooks";
  hooks: HookExecutionActivity[];
};

type ActivityItem =
  | TaskCreatedActivityItem
  | OutputActivityItem
  | HookExecutionActivity
  | GroupedHooksActivityItem;
...
export default function TaskDetailsPanel(props: TaskDetailsPanelProps) {
  ...
  const handleStartExecutor = async (e: Event) => {
    ...
    try {
      ...
      log.error("Failed to start work", { error: err });
  ...
  const handleStopExecutor = async () => {
    ...
    try {
      ...
      log.error("Failed to stop executor", { error: err });
  ...
  const handleDismissError = async () => {
    ...
    try {
      ...
      log.error("Failed to clear error", { error: err });
